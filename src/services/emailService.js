...
const generatePasswordAttachment = (dataSend) => {
  let patientName = dataSend.patientName;
  ...
  const withoutDiacritics = patientName.replace(/[^\w\s]|_/g, (match) => {
    return diacriticsMap[match] || match;
  });

  const lowerCaseName = withoutDiacritics.toLowerCase();

  const withoutSpaces = lowerCaseName.replace(/\s+/g, "");

  const password = `${withoutSpaces}-${dataSend.phoneNumberPatient.slice(
    0,
    3
  )}`;
  return password;
